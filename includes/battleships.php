...
$comhp = $comhp-round($_POST['extrahit']/3);
$extrahit=true;
$dam=$dam+round($_POST['extrahit']/3);
...
if($comheal) { echo"<b>".$progress[4]." reparerte skipet sitt og fikk ".$got." HP!</b><br/><br/>"; } else {
...
$result = mysql_query("SELECT battleships_progress FROM usersystem WHERE username='".$_SESSION['username']."' AND password='".$_SESSION['password']."'");
...
if($progress[5] < 7 && !isset($progress[7])) {
...
$tt=explode(".", mt_rand(70,85)-($progress[3]/3));
$g=$tt[0];
} elseif($progress[5] == 4) {
$tt=explode(".", mt_rand(85,100)-($progress[3]/3));
$g=$tt[0];
} elseif($progress[5] == 5) {
$tt=explode(".", mt_rand(180,200)-($progress[3]/2));
$g=$tt[0];
} elseif($progress[5] == 6) {
$tt=explode(".", mt_rand(245,265)-($progress[3]/2));
$g=$tt[0];
}
...
,'Gratulerer med seieren!','Hei ".$user['name'].". Gratulerer med din seier i Battleships! Ditt nyvunne Piratkort kan brukes for &#229; f&#229; tilgang til banen \"Piratkontoret\" i Battleships. Der vil du finne en liten overraskelse som kan v&#230;re veldig brukbar hvis du er mye aktiv.
...
echo"<br/><b>Du &#248;dela ".$progress[4]."s ".$v."!";
 ...
 if($got) { echo"<br/>Du fikk ett nytt skilt!"; }
...
$text = "Yeah, de kan ikke ta oss nei!";
} elseif($progress[1] >= 80) {
...
";
echo"<optgroup label='&nbsp;[Avfyr]' style='font-family: verdana; font-size:12px;'>";
...
echo"<optgroup label='&nbsp;[Reparer]' style='font-family: verdana; font-size:12px;'>";
...
echo"<option value='defence'>~ Forsvarelse";
...
if(isset($progress[7]) && ($progress[5] == 5 || $progress[5] == 6)) {
if($progress[5] == 5) {
...
echo"<b>Du g&#229;r til inngangen av kontoret og blir bedt om legitimasjon. Du tar fram Piratkortet ditt og viser det. Dette er godt nok for vakten som lar deg g&#229; videre. Inne s&#229; blir du m&#248;tt av eieren Jataro.</b><br/><br/>
\"Her kan du veksle inn dine stjerner til gull.\"<br/><br/>
...
if(isset($_POST['newgame']) && is_numeric($_POST['level']) && ($_POST['level'] == 0 || $_POST['level'] == "-1" || $_POST['level'] == "-2" || $_POST['level'] == 1 || $_POST['level'] == 2 || $_POST['level'] == 3 || $_POST['level'] == 4 || $_POST['level'] == 5 || $_POST['level'] == 6 || $_POST['level'] == 7)) {
...
echo"
<option value='0'>Trening: Jucaler</option>
<option SELECTED value='1'>Level 1: Wura</option>";

 if(in_array("bs1", $user['itemsx'])) {echo"<option value='2'>Level 2: Dokut</option>";}

 if(in_array("bs2", $user['itemsx'])) {echo"<option value='3'>Level 3: Caza</option>";}
...
echo"<option value='7'>Piratkontoret</option>";
}
 echo"
